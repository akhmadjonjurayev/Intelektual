...
namespace Intelektual_Tizimlar
{
    internal static class SeedData
    {
        ...
        //    //data.Add(new Koordinate(new double[] { 6, -1 }, 1));
        //    //data.Add(new Koordinate(new double[] { -1, 10 }, 1));
        //    //data.Add(new Koordinate(new double[] { 2, 3 }, 1));

        //    //data.Add(new Koordinate(new double[] { 0, -11 }, 2));
        //    //data.Add(new Koordinate(new double[] { -9, -8 }, 2));
        //    //data.Add(new Koordinate(new double[] { 3, 11 }, 2));

        //    return data;
        //}

        public static List<Koordinate> GetDataFromIris()
        {
            var data = new List<Koordinate>();
            var path = string.Format("{0}\\Data\\iris.data", Directory.GetCurrentDirectory());
            using (var stream = File.OpenText(path))
            {
                ...
                {
                    var cors = line.Split(',');
                    string classNumber = cors.Last();
                    var cor = GetCoordinate(cors.Take(cors.Length - 1)).ToArray();
                    if(cor.Length == 0)
                    {
                        var check_name = classNumber;
                    }
                    data.Add(new Koordinate(cor, classNumber));
                }
            }
